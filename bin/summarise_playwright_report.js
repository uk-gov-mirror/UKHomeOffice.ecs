...
function buildSummaryFromText(outputText) {
  ...
  return ['Playwright Nightly Summary', `Total: ${total}`, `Passed: ${passed}`, `Failed: ${failed}`, `Flaky: ${flaky}`, `Skipped: ${skipped}`, `Duration: ${parseDuration(outputText)}`].join('\n');
}
...
let summary;
try {
  summary = buildSummaryFromText(readText(inputPath));
