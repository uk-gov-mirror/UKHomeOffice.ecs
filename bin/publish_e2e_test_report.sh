...
RELEASE_HTTP_CODE="$(curl -sS -o "$RELEASE_RESPONSE_FILE" -w "%{http_code}" -H "Authorization: Bearer ${GITHUB_APP_TOKEN}" -H "Accept: application/vnd.github+json" "${GH_API}/releases/tags/${TAG_NAME}")"

if [ "$RELEASE_HTTP_CODE" = "404" ]; then
  RELEASE_HTTP_CODE="$(curl -sS -o "$RELEASE_RESPONSE_FILE" -w "%{http_code}" -X POST -H "Authorization: Bearer ${GITHUB_APP_TOKEN}" -H "Accept: application/vnd.github+json" "${GH_API}/releases" -d "$(jq -n --arg tag "$TAG_NAME" --arg name "Playwright Report PR #${DRONE_PULL_REQUEST}" '{tag_name:$tag,name:$name,prerelease:true,draft:false}')")"
fi

if ! [[ "$RELEASE_HTTP_CODE" =~ ^2 ]] || ! jq -e . "$RELEASE_RESPONSE_FILE" >/dev/null 2>&1; then
...
UPLOAD_HTTP_CODE="$(curl -sS -o "$ASSET_RESPONSE_FILE" -w "%{http_code}" -X POST -H "Authorization: Bearer ${GITHUB_APP_TOKEN}" -H "Content-Type: application/zip" --data-binary @"${REPORT_ARCHIVE}" "https://uploads.github.com/repos/${DRONE_REPO}/releases/${RELEASE_ID}/assets?name=${REPORT_ARCHIVE}")"
if ! [[ "$UPLOAD_HTTP_CODE" =~ ^2 ]] || ! jq -e . "$ASSET_RESPONSE_FILE" >/dev/null 2>&1; then
...
rm -f "$ASSET_RESPONSE_FILE"
test -n "$DOWNLOAD_URL" || (echo "Upload succeeded but browser_download_url is missing" && exit 1)

COMMENT_BODY="Playwright HTML report for this PR build is available here:
* ${DOWNLOAD_URL}

Drone build: ${DRONE_BUILD_LINK}"
GH_COMMENT_RESPONSE="$(curl -sS -X POST -H "Authorization: Bearer ${GITHUB_APP_TOKEN}" -H "Accept: application/vnd.github+json" "${GH_API}/issues/${DRONE_PULL_REQUEST}/comments" -d "$(jq -n --arg body "$COMMENT_BODY" '{body: $body}')")"
GH_COMMENT_URL="$(echo "$GH_COMMENT_RESPONSE" | jq -r '.html_url // empty')"
test -n "$GH_COMMENT_URL" || (echo "Failed to create GitHub PR comment" && echo "$GH_COMMENT_RESPONSE" && exit 1)
